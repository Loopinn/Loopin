...
onBeforeMount(async () => {
  const { data, error } = await supabase.from("userinfo").select().eq("nickname", "2테스트용");
...
<template>
  <div v-if="currentUser" class="px-5 flex flex-col gap-10">
    <div>
      <h2 class="font-extrabold text-[22px]">프로필 변경</h2>
      <div class="relative w-[75px] h-[75px]">
        ...
        />
        <img v-else-if="currentUser.profile_img && preImg" :src="preImg" alt="" class="w-full h-full rounded-full" />
